...
resource "launchdarkly_project" "devopsdays" {
  key  = "ld-demo-devopsdays"
  name = "ld-demo-devopsdays"
...
resource "launchdarkly_feature_flag" "qrcode" {
  project_key = launchdarkly_project.devopsdays.key
  key         = "qrcode"
  name        = "0 - QR Code"
...
output "LaunchDarkly_API_Key" {
  value = launchdarkly_project.devopsdays.environments[0].api_key
  sensitive = true
}

output "LaunchDarkly_Client_Side_Key" {
  value = launchdarkly_project.devopsdays.environments[0].client_side_id
  sensitive = true
}
